...
EOF
  ...
  # TODO: investigate why the process of adding nodes must be serial for centos.
  # With ubuntu, the worker nodes can be setup in parallel, which reduces the
  # time needed to install. But with centos, there seems to be some race
  # condition between the 1st and 2nd nodes added, which causes the 1st node
  # to work incorrectly post-Ready. Symptom is that nodeports cannot be routed
